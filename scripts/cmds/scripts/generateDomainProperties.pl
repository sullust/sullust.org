...
foreach my $sEntry (@smartsEntries) {
	...
	foreach my $snmcpeServer(@serverList) { 	
		...
		if($snmcpeServer =~ m/$domain-/i) { 
			...
			`sed 's/SNMPTRAPDESTINATION.*/SNMPTRAPDESTINATION=$smartsServer/' properties/$domainProperties.$domain > properties/tmp`;	
			`mv properties/tmp properties/$domainProperties.$domain`;
		}
	}
