...
if [[ $IMPORT_PORT_MAP -eq 1 ]]; then
		scp -i /home/jainswor/.ssh/id_rsa jainswor@pdcc01-cmds1.mso.mci.com:~/staging/port-map /tmp/port-map.$$;
else
		touch /tmp/port-map.$$
fi
...
if [[ $IMPORT_PORT_MAP -eq 1 ]]; then
	scp -i /home/jainswor/.ssh/id_rsa /tmp/port-map.$$ jainswor@pdcc01-cmds1.mso.mci.com:~/staging/port-map
fi
