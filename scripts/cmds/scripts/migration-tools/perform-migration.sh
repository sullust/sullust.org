...
if [[ $SNMCPE_MIGRATION -eq 1 ]]; then
	./instance-copy/SNMCPE-Instance-Copy.sh $FROM $TO
	./instance-copy/Migrate-SNMCPE-Instance.sh $FROM $TO
	exit 0
fi;
...
if [[ $IS_SHAKEOUT == "true" ]]; then
	doPrepareShakeout $IMPORT_PORT_MAP;
	doDeployShakeout;
fi
