...
my $BASE_PROJECT_DIR=$ENV{'BASE_PROJECT_DIR'}; 
my $SHAKEOUT_ESPWS_SERVER=$ENV{'SHAKEOUT_ESPWS_SERVER'};
my $SHAKEOUT_TOOLS_SERVER=$ENV{'SHAKEOUT_TOOLS_SERVER'};
my $FROM=$ENV{'FROM'};
my $TO=$ENV{'TO'};
...
my ($availablePortsFile, $portMapFile) = @ARGV;

open(BP_DIR, "$BASE_PROJECT_DIR");
open(SUMMARY, "> /tmp/summary.$$");
...
foreach my $file(@all) {
	...
	if($file !~ m/(cpe|extra|timeout)/) {
		...
		while(<FH>) {
			...
			if(($line =~ m/TOOLS_/ || $line =~ m/^espws\./ 
				|| $line =~ m/^SECURE_PORT=/) && $line !~ m/(PRINCIPLE|CREDENTIALS)/) {
				($port) = ($line =~ m/([0-9]{4,4})$/);
			...
			if($port =~ m/./) {
					if($filePrinted{$file} == 0) {
						...
						if($file =~ m/config.xml/) {
							my $filePath = $file;
							$filePath =~ s/\/config\/config.xml.*//g;
							print SUMMARY $filePath."\n";
						}
					}
					...
					print SUMMARY "\t$port -> $portMap{$port}\n" unless $file !~ m/config.xml/;
