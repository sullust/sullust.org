...
for (my $i = 1; $i <= $PBIRowCount; $i++) {
	...
	foreach my $task (@{$taskHash{$PBIid}}) {
		...
		print "Sprint Begin Date: $tHash{sprintBeginDate}\n";
		print "Sprint End Date: $tHash{sprintEndDate}\n";
		
		my ($bYear, $bMonth, $bDay) = split(/-/, $tHash{sprintBeginDate});
		my ($eYear, $eMonth, $eDay) = split(/-/, $tHash{sprintEndDate});
		...
		$bcWkS->write_string($totalRowCount, 3, $tHash{productName}); #PBIRow);
		...
		$bcWkS->write_string($totalRowCount, 8, '?');
		...
		$bcWkS->write_string($totalRowCount, 9, '?');
		...
		$bcWkS->write_string($totalRowCount, 13, '?');
		#$bcWkS->write_string($totalRowCount, 14, 'Becky Beasley');
		$bcWkS->write_string($totalRowCount, 14, '?');
		#$bcWkS->write_string($totalRowCount, 15, 'Kevin Cropp');
		$bcWkS->write_string($totalRowCount, 15, '?');
		...
		$bcWkS->write_string($totalRowCount, 19, '?');
...
sub buildTaskHash() {
	...
	my $productInfoSheet = $oBook->worksheet('Product Info');
		
	for (my $i = 1; $i < $oWkS->{MaxRow}; $i++) {
		my %task = ();
		print "$i ";
		print "a";
		my $productName = $productInfoSheet->get_cell(1, 0)->value();
